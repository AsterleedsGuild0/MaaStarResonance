...
@AgentServer.custom_action("DodgeTheBallPoint")
class DodgeTheBallPointAction(CustomAction):
    ...
    def run(
        ...
        logger.info(f"本次任务设置的最大浪花躲避球次数: {max_game_count if max_game_count != 0 else '无限'}")

        while not context.tasker.stopping:
            logger.info(f"=== 已成功浪花躲避球 {self.game_count} 次 ===")
            # 检查是否已经游戏足够次数了
...
def ensure_into_ball(context: Context) -> bool:
    """
    确保进入浪花躲避球
    """
    while not context.tasker.stopping:
    ...
    logger.error("浪花躲避球被手动中止！")
    return False


def ensure_ball_entry(context: Context, timeout: int = 120) -> bool:
    """确保到达浪花躲避球的入口"""
    # 先检测一下是否可以直接进
    ...
    # 第二次还不行才导航过去
    teleport_or_navigate(context, "游星岛", "浪花躲避球", "导航", NAVIGATE_DATA)
    ...
    while elapsed_time <= timeout and not context.tasker.stopping:
        ...
        if check_is_entry(context):
            logger.info(f"检测到已经到达浪花躲避球的入口！")
            return True
        time.sleep(2)
    logger.error(f"超 {timeout} 秒未到达浪花躲避球的入口 或 小游戏暂未开启！")
    return False
