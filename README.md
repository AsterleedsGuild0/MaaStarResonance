<!-- markdownlint-disable MD041 MD034 -->
<div align="center">
<img alt="LOGO" src="static/img/logo_transparent.png" width="256" height="256" />

# 星痕共鸣 MAA 小助手

基于图像识别的星痕共鸣自动化工具

由 [MaaFramework](https://github.com/MaaXYZ/MaaFramework) 强力驱动！  

<a
  href="https://github.com/AsterleedsGuild0/MaaStarResonance"
  target="_blank"
  style="font-weight: bold;"
>
  🔗 本项目 GitHub 仓库
</a><br>
<a
  href="https://maa-star-resonance.ayusummer233.top"
  target="_blank"
  style="font-weight: bold;"
>
  🔗 使用文档
</a><br>

</div>

<p align="center">
  <img alt="Python" src="https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white">
  <img alt="platform" src="https://img.shields.io/badge/platform-Windows-blueviolet">
  <img alt="license" src="https://img.shields.io/github/license/MAA1999/M9A">
  <br>
  <img alt="commit" src="https://img.shields.io/github/commit-activity/m/AsterleedsGuild0/MaaStarResonance">
  <img alt="stars" src="https://img.shields.io/github/stars/AsterleedsGuild0/MaaStarResonance?style=social">
  <img alt="downloads" src="https://img.shields.io/github/downloads/AsterleedsGuild0/MaaStarResonance/total?style=social">
</p>

---

## 界面预览

- 如果觉得还算好用请给我们的项目点个收藏支持一下⭐

![image-20260109134752](docs/关于/关于我们.assets/image-20260109134752.png)

---

## 声明与许可

- 本项目采用 [`AGPL-3.0`](LICENSE) 许可证进行开源
- 本资源按"现状"提供，不附带任何形式的明示或暗示担保。使用者需自行承担所有风险
- 作者不对因使用本资源而导致的任何直接、间接或结果性损失承担责任
- 基于本资源进行的任何商业行为均与原作者无关，使用者不得暗示任何官方关联或认可
- 项目图标来源: [艾露娜Q版表情包全套高清修复分享｜星痕共鸣｜腾讯频道](https://pd.qq.com/g/pd34199182/post/B_e60f346878db0d001441152189808449200X60)

---

## 功能列表

> ⚠️ 目前仅支持 `MuMu模拟器` 运行星痕共鸣 接入本 Maa 应用程序，支持后台运行，支持多开
>
> 您可以采用多个配置页、复制、排列、组合任务列表的方式来完成日常

- 自动钓鱼
- 简易采集
- 完成一次不稳定空间
- 领取今日活跃度奖励
- 挂机自动批复入队申请
- 聊天频道发言
- 周期性聊天频道发言
- 游星岛暴打陈敏小游戏
- 游星岛躲猫猫小游戏（队员简易版）
- 游星岛躲猫猫小游戏（完整版开发中暂不支持）
- 游星岛镭射回旋小游戏
- 游星岛极限落潮小游戏
- 游星岛浪花躲避球小游戏
- 游星岛麻将挂机小游戏
- 游星岛环城载具赛小游戏
- Lv10浮游之茧(通用刷茧暂停开发)
- 自动接受协会狩猎邀请和战斗
- 关闭所有广告弹窗
- 地图传送
- 地图导航
- 切换分线

---

## 文档

- 在线文档入口: [星痕共鸣 Maa 小助手](https://maa-star-resonance.ayusummer233.top)
- 新手指南入口: [新手指南入口](./docs/新手指南/新手指南入口.md)
- 功能文档入口: [功能文档入口](./docs/功能文档/功能文档入口.md)
- 开发文档入口: [开发者文档入口](./docs/开发者文档/开发者文档入口.md)
- 关于我们: [关于我们](./docs/关于/关于我们.md)

## 新手指南

- [新手指南入口](./docs/新手指南/新手指南入口.md)
- [环境准备](./docs/新手指南/环境准备.md)
- [模拟器配置](./docs/新手指南/模拟器配置.md)
- [连接程序与确认实例](./docs/新手指南/连接程序与确认实例.md)
- [开始任务](./docs/新手指南/开始任务.md)
- [常见问题](./docs/新手指南/常见问题.md)

---

## 功能文档

- [功能文档入口](./docs/功能文档/功能文档入口.md)
- [任务流说明入口](./docs/用户文档/任务流说明/任务流说明入口.md)
- [自动钓鱼](./docs/用户文档/任务流说明/自动钓鱼/自动钓鱼使用须知.md)
- [简易采集](./docs/用户文档/任务流说明/简易采集/简易采集使用须知.md)
- [自动接受协会狩猎邀请和战斗](./docs/用户文档/任务流说明/自动协会狩猎/使用须知.md)
- [聊天频道发言](./docs/用户文档/任务流说明/聊天频道发言/聊天频道发言使用须知.md)
- [Lv10 浮游之茧](./docs/用户文档/任务流说明/S2自动刷茧/lv10茧/S2自动刷lv10茧使用须知.md)

---

## 开发文档

- [QuickStart](./docs/开发者文档/QuickStart.md)
- [开发者文档入口](./docs/开发者文档/开发者文档入口.md)
- [个性化配置](./docs/开发者文档/个性化配置.md)
- [文档编写规范](./docs/开发者文档/文档编写规范.md)
- [文档维护流程](./docs/开发者文档/文档维护流程.md)
- [其他开发随笔](./docs/开发者文档/开发随笔.md)
- [部分功能流程设计](./docs/开发者文档/流程设计/)
- [立项初期的待办清单](./docs/开发者文档/待办清单.md)

更多开发文档请前往 [MaaFramework](https://github.com/MaaXYZ/MaaFramework) 仓库查看

---

## 鸣谢

- 本项目由 **[MaaFramework](https://github.com/MaaXYZ/MaaFramework)** 强力驱动！  
- 软件UI 由 [MFAAvalonia](https://github.com/SweetSmellFox/MFAAvalonia)  大力支持！
- 感谢以下开发者对本项目作出的贡献:

[![Contributors](https://contrib.rocks/image?repo=AsterleedsGuild0/MaaStarResonance&max=1000)](https://github.com/AsterleedsGuild0/MaaStarResonance/graphs/contributors)

---

## 协会招募

阿斯特里斯零协会（ID：14130）招人啦~

保持活跃即可，有协会Q群，有OOPZ语音频道，每周有团本车~

协会群里有玩 084、二重、PCR、MC、战地 等各种游戏的群友，欢迎加入我们协会，~~一起带薪群聊~~

协会直接搜ID进即可，也欢迎其他协会前来合会~

![image-20251125113159](docs/关于/关于我们.assets/image-20251125113159.png)

## 友情链接

- [另一款 MAA Star Resonance](https://github.com/26F-Studio/maa-star-resonance)
  这是一个由 [Particle_G](https://github.com/ParticleG) 开发的星痕共鸣MAA小助手，使用 Electron + 文本图像识别 + ADB模拟控制 技术实现
