...
class TcpReassembler:
    ...
    Identifies game server endpoints by signature detection, then
    reassembles TCP streams from that server into ordered byte data.
    """
    ...
    def __init__(self, on_data: Callable[[bytes], None]) -> None:
        ...
        self._server_endpoint: ServerEndpoint | None = None
        self._on_data = on_data

    @property
    def server_endpoint(self) -> ServerEndpoint | None:
        """The detected game server endpoint, or None if not yet detected."""
    ...
    def process_packet(
        ...
        # Try server detection if not yet identified
        if self._server_endpoint is None:
            if self._detect_server(src_ip, src_port, payload, seq, len(payload)):
                return

        # Only process packets FROM the game server
        if self._server_endpoint is None:
            return
        if src_ip != self._server_endpoint.ip or src_port != self._server_endpoint.port:
            return
    ...
    def _detect_server(
        ...
        Uses two signature methods from the C# reference:
        - Method A: server signature bytes at a specific offset in nested frames
        - Method B: login return packet with exact byte pattern
    ...
    def _set_server(self, ip: str, port: int, seq: int, payload_len: int) -> None:
        ...
        """
        self._server_endpoint = ServerEndpoint(ip=ip, port=port)
