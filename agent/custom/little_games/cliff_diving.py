...
@AgentServer.custom_action("CliffDivingPoint")
class CliffDivingPointAction(CustomAction):
    ...
    def run(
        ...
        logger.info(f"本次任务设置的最大极限落潮次数: {max_game_count if max_game_count != 0 else '无限'}")

        # 第一次小游戏前所需切换的分线
        need_line = get_game_need_line(context)

        while not context.tasker.stopping:
            logger.info(f"=== 已成功极限落潮 {self.game_count} 次 ===")
            # 检查是否已经游戏足够次数了
...
def ensure_into_diving(context: Context) -> bool:
    """
    确保进入极限落潮
    """
    while not context.tasker.stopping:
    ...
    logger.error("极限落潮被手动中止！")
    return False


def ensure_diving_entry(context: Context, timeout: int = 120) -> bool:
    """确保到达极限落潮的入口"""
    # 先检测一下是否可以直接进
    if check_is_entry(context):
        return True

    # 第一次不行的话尝试向左前方走几步 | 可能是上一把刚结束的情况，走几步就能再次到入口
    ...
    # 第二次还不行才导航过去
    teleport_or_navigate(context, "游星岛", "极限落潮", "导航", NAVIGATE_DATA)
